...
def auth_screen():
    def auth_screen():
    # 1. The Massive App Name & Subtitle
        st.markdown("<h1 style='text-align:center; font-size: 3.5rem; color:#5E2CA5; font-weight: 800; margin-bottom: 0px;'>MediCap</h1>",      unsafe_allow_html=True)
        st.markdown("<p style='text-align:center; color:#8C8C8C; font-size: 1.2rem; margin-bottom: 40px;'>Healthcare Capacity Management System</p>", unsafe_allow_html=True)
    
    # 2. Use columns to perfectly center the login box on the screen
    col1, col2, col3 = st.columns([1, 1.2, 1])
    
    with col2: # Everything inside here stays in the middle column
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        
        # The Toggle Switch
        mode = st.radio("Access Mode", ["Login", "Sign Up"], horizontal=True, label_visibility="collapsed")
        st.markdown("<br>", unsafe_allow_html=True) # Add a tiny bit of space
        
        if mode == "Sign Up":
            name = st.text_input("Hospital Name", placeholder="e.g. City General")
            h_id = st.text_input("Hospital ID (CMS)", placeholder="CMSXXXXX")
            email = st.text_input("Business Official Email", placeholder="admin@hospital.com")
            region = st.selectbox("Facility Region", ["Karnataka", "Maharashtra", "Delhi", "Tamil Nadu", "Other"])
            pw = st.text_input("Create Password", type="password")
            
            # Make sure we add some space before the button
            ...
            h_id = st.text_input("Hospital ID", placeholder="Enter your CMS ID")
